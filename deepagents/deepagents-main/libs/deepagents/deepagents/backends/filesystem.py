...
class FilesystemBackend(BackendProtocol):
    ...
    def _strip_win_extended_prefix(p: Path) -> Path:
        ...
        ``Path.resolve()`` on Windows sometimes returns paths prefixed with
        ``\\\\?\\`` (e.g. ``\\\\?\\D:\\foo``).  ``Path.relative_to()`` does an
        exact prefix comparison, so a path *with* the prefix will fail the
        containment check against a ``cwd`` stored *without* it (or vice-versa).
        Stripping the prefix from both sides before the comparison keeps the
        security check correct while avoiding spurious rejections.

        Args:
            p: Path to normalize.

        Returns:
            Path with the ``\\\\?\\`` prefix removed when present; ``p`` unchanged
            on non-Windows platforms or when the prefix is absent.
        """
        s = str(p)
        if s.startswith("\\\\?\\"):
            return Path(s[4:])
    ...
    def _resolve_path(self, key: str) -> Path:
        ...
        if self.virtual_mode:
            ...
            # Normalize the Windows \\?\ extended-length prefix before comparing —
            # Path.resolve() may add it on deep paths while self.cwd was stored
            # without it, causing a spurious "outside root directory" rejection.
            try:
                self._strip_win_extended_prefix(full).relative_to(
                    self._strip_win_extended_prefix(self.cwd)
                )
            except ValueError:
